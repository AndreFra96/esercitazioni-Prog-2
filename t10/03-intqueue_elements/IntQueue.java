...
public class IntQueue {
    ...
    public Iterator<Integer> elements() {
        return new QueueElementGenerator(this);
    }

    /** GENERATORE */
    static class QueueElementGenerator implements Iterator<Integer> {
        IntQueue q;
        int index;
        int count; 

        QueueElementGenerator(IntQueue queue) {
            this.q = queue;
            index = q.head;
        }
        ...
        @Override
        public Integer next() {
            Integer next = q.elements[index];
            index = (index + 1) % q.elements.length;
            count++;
            return next;
        }
    }
