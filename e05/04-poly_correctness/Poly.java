...
public class Poly {
     ...
     * q.poly[1]x^1 + ... + q.poly[q.poly.length - 1]x^q.poly.length - 1) è uguale
     * al polinomio il quale coefficente di grado x è uguale al coefficente di grado
     * x di this (o 0 se il this ha grado inferiore a x) moltiplicato per il coefficente del
     * grado x di q (o 0 se il q ha grado inferiore a x) per ogni x compresa fra 0 e
     * il grado del polinomio di grado maggiore.
     */
     ...
     * 
     * Correttezza:  -(this.poly[0]x^0 + this.poly[1]x^1 + ... +
     * this.poly[this.poly.length - 1]x^this.poly.length - 1) = -this.poly[0]x^0 - this.poly[1]x^1 - ... -
     * this.poly[this.poly.length - 1]x^this.poly.length - 1
     */
    public Poly minus() {
    ...
    @Override
    public int hashCode() {
        // return toString().hashCode(); //Posso farlo? (edit: ASSOLUTAMENTE NO!!)
